...
PerformBaseExperiments <- function(dfs, all.genes, parallel_ = TRUE){
  ...
  for (gene in all.genes) {
    ...
    # Train and log model.
    # ILP
    lasso.ilp.model <- FitLasso(base.ilp.train, y.train)
    ...
    # FP
    lasso.fp.model <- FitLasso(base.fp.train, y.train)
    ...
    # ILP + FP
    lasso.ilp.fp.model <- FitLasso(base.ilp.fp.train, y.train)
    ...
    lasso.avg <- GetPerformanceMetrics(y.test, merged.preds)
